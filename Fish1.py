...
async def elevenlabs_stream(text_iterator):
    ...
    async with websockets.connect(uri) as ws:
        await ws.send(json.dumps({
            ...
            "generation_config": {"chunk_length_schedule": [50]},  # 🍇 Smooth early streaming
        ...
        async def play_audio(audio_stream):
            ...
            try:
                async for chunk in audio_stream:
                    if interrupt_requested and chunk_index > 10:  # 🍓 Don't interrupt too early
        ...
        async def audio_listener():
            while True:
                ...
                if audio_b64:
                    yield base64.b64decode(audio_b64)  # ✅ This is raw PCM audio now
                elif data.get("isFinal"):
                    break

        # === Interrupt Listener (VAD) ===
        async def listen_for_interrupt():
            global interrupt_requested
            await asyncio.sleep(1.2)  # 🍓 Let Billy start speaking before listening
            ...
            try:
                while not interrupt_requested:
                    ...
                    is_speech = vad.is_speech(chunk, sample_rate) and volume > 1500  # 🍋 Less sensitive
                    if is_speech:
        ...
        # === Start playback, animation, and interruption tasks ===
        audio_task = asyncio.create_task(play_audio(audio_listener()))
        anim_task = asyncio.create_task(animate_billy())
        interrupt_task = asyncio.create_task(listen_for_interrupt())

        # Send text in streaming chunks
